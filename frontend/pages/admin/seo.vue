...
<template>
  <div>
    ...
    <div v-else>
    <h1 class="text-3xl font-bold mb-8">SEO/AEO/GEO 優化管理</h1>
    
    <!-- SEO 基本設定 -->
    <div class="card mb-8">
      <h2 class="text-2xl font-semibold mb-6">SEO 基本設定</h2>
      
      <form @submit.prevent="saveSeoSettings" class="space-y-4" accept-charset="utf-8">
        <div>
          ...
          />
          <p class="text-sm text-gray-500 mt-1">建議長度：30-60 個字元</p>
        </div>
        ...
        <div>
          ...
          ></textarea>
          <p class="text-sm text-gray-500 mt-1">建議長度：120-160 個字元</p>
        </div>
        ...
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">使用逗號分隔多個關鍵字</p>
            <div
              v-if="showKeywordSuggest"
              class="mt-2 bg-gray-100 border rounded p-2"
            >
              <div class="mb-2 text-xs text-gray-600">熱門關鍵字建議：</div>
          ...
          <button type="submit" class="btn-primary">儲存 SEO 設定</button>
      </form>
    </div>
    
    <!-- AEO 設定 -->
    <div class="card mb-8">
      <h2 class="text-2xl font-semibold mb-6">AEO 答案引擎優化</h2>
      
      <form @submit.prevent="saveAeoSettings" class="space-y-4" accept-charset="utf-8">
        <div>
          ...
          ></textarea>
          <p class="text-sm text-gray-500 mt-1">回答常見問題，40-60 字最佳</p>
        </div>
        ...
        <div>
          ...
          <div class="space-y-3">
            ...
            <button type="button" @click="addFaq" class="text-primary">
              + 新增 FAQ
            </button>
          </div>
        </div>
        
          <button type="submit" class="btn-primary">儲存 AEO 設定</button>
      </form>
    </div>
    
    <!-- GEO 設定 -->
    <div class="card mb-8">
      <h2 class="text-2xl font-semibold mb-6">GEO 地理優化</h2>
      
      <form @submit.prevent="saveGeoSettings" class="space-y-4" accept-charset="utf-8">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <button type="submit" class="btn-primary">儲存地理設定</button>
      </form>
    </div>
    
    <!-- 社群媒體設定 -->
    <div class="card">
      <h2 class="text-2xl font-semibold mb-6">社群媒體連結</h2>
      
      <form @submit.prevent="saveSocialSettings" class="space-y-4" accept-charset="utf-8">
        <div>
        ...
        <div>
          ...
          <button type="submit" class="btn-primary">儲存社群設定</button>
      </form>
      </div>
    </div>
  </div>
...
function addKeyword(kw: string) {
  const arr = seoSettings.keywords
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  if (!arr.includes(kw)) arr.push(kw);
  seoSettings.keywords = arr.join(", ");
}

const addFaq = () => {
  aeoSettings.faqs.push({ question: "", answer: "" });
};

const removeFaq = (index: number) => {
  aeoSettings.faqs.splice(index, 1);
};
...
const generateAIFaq = async () => {
  ...
  try {
    const { faqs } = await $fetch("/api/ai-gemini", {
      ...
      body: {
        type: "faq",
        content: `${seoSettings.title} ${seoSettings.description} ${seoSettings.keywords}`,
      },
    });
    if (faqs) aeoSettings.faqs = faqs;
  } catch (e) {
    alert("AI FAQ 產生失敗");
  }
...
const generateAIMeta = async () => {
  ...
  try {
    const { description } = await $fetch("/api/ai-gemini", {
      ...
      body: {
        type: "meta",
        content: `${seoSettings.title} ${seoSettings.keywords}`,
      },
    });
    if (description) seoSettings.description = description;
  } catch (e) {
    alert("AI meta 產生失敗");
  }
...
const generateAIAlt = async () => {
  ...
  try {
    const { alt } = await $fetch("/api/ai-gemini", {
      ...
      body: {
        type: "alt",
        content: `${seoSettings.title} ${seoSettings.description}`,
      },
    });
    if (alt) seoSettings.alt = alt;
  } catch (e) {
    alert("AI alt 產生失敗");
  }
...
const generateAIKeywords = async () => {
  ...
  try {
    ...
    if (keywords) seoSettings.keywords = keywords;
  } catch (e) {
    alert("AI 關鍵字產生失敗");
  }
