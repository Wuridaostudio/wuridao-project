...
export class LogsController {
  ...
  async receiveFrontendLog(@Body() logEntry: FrontendLogEntry) {
    this.logger.log(`📱 [前端日誌] [${logEntry.component}] [${logEntry.level.toUpperCase()}] ${logEntry.message}`, {
      ...logEntry,
      source: 'frontend',
    });
  ...
  async receiveFrontendLogs(@Body() request: BatchLogRequest) {
    ...
    request.logs.forEach((logEntry, index) => {
      this.logger.log(`📱 [前端日誌 ${index + 1}] [${logEntry.component}] [${logEntry.level.toUpperCase()}] ${logEntry.message}`, {
        ...logEntry,
        source: 'frontend',
        batchIndex: index + 1,
      });
