    ...
    <div class="seo-analyzer bg-gray-800 rounded-lg p-4 border border-gray-700">
      ...
      <div class="mb-6">
        <div class="flex items-center justify-between mb-2">
          <h3 class="text-lg font-semibold text-white">SEO 分數</h3>
          <div class="text-sm text-gray-400">{{ lastUpdated }}</div>
        </div>
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="text-sm text-gray-300">{{ scoreText }}</div>
            <div class="text-xs text-gray-400">{{ scoreDescription }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-3 mb-6">
        <div class="bg-gray-700 rounded p-3">
          <div class="text-2xl font-bold text-white">{{ stats.wordCount }}</div>
          <div class="text-xs text-gray-400">字數</div>
        </div>
        <div class="bg-gray-700 rounded p-3">
          <div class="text-2xl font-bold text-white">{{ stats.imageCount }}</div>
          <div class="text-xs text-gray-400">圖片</div>
        </div>
        <div class="bg-gray-700 rounded p-3">
          <div class="text-2xl font-bold text-white">{{ stats.linkCount }}</div>
          <div class="text-xs text-gray-400">連結</div>
        </div>
        <div class="bg-gray-700 rounded p-3">
          <div class="text-2xl font-bold text-white">
            {{ stats.headingCount }}
          </div>
          <div class="text-xs text-gray-400">標題</div>
        </div>
      </div>

      <!-- SEO 建議 -->
      <div class="space-y-4">
        <h4 class="text-md font-semibold text-white">SEO 建議</h4>
        ...
        <div v-if="suggestions.success.length > 0" class="space-y-2">
          ...
          >
            ...
            <div class="text-sm text-green-300">{{ suggestion.message }}</div>
          </div>
        </div>
        ...
        <div v-if="suggestions.warnings.length > 0" class="space-y-2">
          ...
          >
            ...
            <div class="text-sm text-yellow-300">{{ suggestion.message }}</div>
          </div>
        </div>
        ...
        <div v-if="suggestions.errors.length > 0" class="space-y-2">
          ...
          >
            ...
            <div class="text-sm text-red-300">{{ suggestion.message }}</div>
          </div>
        </div>
      ...
      <div v-if="keywordAnalysis.length > 0" class="mt-6">
        <h4 class="text-md font-semibold text-white mb-3">關鍵字分析</h4>
        <div class="space-y-2">
          <div
          ...
          >
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs text-gray-400"
                >{{ keyword.density.toFixed(1) }}%</span
              >
            </div>
          </div>
        </div>
...
const scoreColor = computed(() => {
  if (seoScore.value >= 80) return "text-green-500";
  if (seoScore.value >= 60) return "text-yellow-500";
  return "text-red-500";
});

const scoreText = computed(() => {
  if (seoScore.value >= 80) return "優秀";
  if (seoScore.value >= 60) return "良好";
  if (seoScore.value >= 40) return "一般";
  return "需改善";
});

const scoreDescription = computed(() => {
  if (seoScore.value >= 80) return "SEO 表現很好！";
  if (seoScore.value >= 60) return "還有改善空間";
  if (seoScore.value >= 40) return "需要更多優化";
  return "建議大幅改善";
});
